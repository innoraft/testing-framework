...
public class ScreenshotFunctions {
    ...
    public void Folder_Setup(String Sitelevel1, String Sitelevel2, String Site, String Path) throws IOException {
        // Create folder for screenshot comparision
        ...
        // If number of folders in Sitelevel1 folder is greater than or equal to number of folders in Sitelevel2 folder
        if (Sitelevel1FolderListOfFiles.length <= Sitelevel2FolderListOfFiles.length) {
            for (int i=0 ; i < Sitelevel1FolderListOfFiles.length ; i++){
                // Create folder in screenshot comparision folder
                ...
                // If number of files in folder of Sitelevel1 folder equal to number of files in folder of Sitelevel2 folder
                if (SiteLevel1FolderFiles.length == SiteLevel2FolderFiles.length) {
                    for (int j=0 ; j < SiteLevel1FolderFiles.length ; j++){
                        // To construct the path of file.
                        Path original = Paths.get(BasePath + Path + Sitelevel1 + "\\" + Sitelevel1FolderListOfFiles[i].getName() + "\\" + SiteLevel1FolderFiles[j].getName());
                    ...
                    for (int k=0 ; k < SiteLevel2FolderFiles.length ; k++){
                        // To construct the path of file.
                        Path original = Paths.get(BasePath + Path + Sitelevel2 + "\\" + Sitelevel2FolderListOfFiles[i].getName() + "\\" + SiteLevel2FolderFiles[k].getName());
                        Path destination = Paths.get(BasePath + Path + Site + "_" + Sitelevel1 + "_" + Sitelevel2 + "\\" + Sitelevel1FolderListOfFiles[k].getName() + "\\" + SiteLevel2FolderFiles[k].getName());
                        // To move file from one directory to another and if same name file exist in directory than it will replace.
                        Files.copy(original, destination, StandardCopyOption.REPLACE_EXISTING);
                    }
                }
            ...
            for (int i=0 ; i < Sitelevel2FolderListOfFiles.length ; i++){
                // Create folder in screenshot comparision folder
                File ScreenshotDirectory = new File(BasePath + Path + Site + "_" + Sitelevel1 + "_" + Sitelevel2 + "\\" + Sitelevel1FolderListOfFiles[i].getName());
                ScreenshotDirectory.mkdir();
                ...
                if (SiteLevel1FolderFiles.length == SiteLevel2FolderFiles.length) {
                    for (int j=0 ; j < SiteLevel1FolderFiles.length ; j++){
                        // To construct the path of file.
                        Path original = Paths.get(BasePath + Path + Sitelevel1 + "\\" + Sitelevel1FolderListOfFiles[i].getName() + "\\" + SiteLevel1FolderFiles[j].getName());
                        Path destination = Paths.get(BasePath + Path + Site + "_" + Sitelevel1 + "_" + Sitelevel2 + "\\" + Sitelevel1FolderListOfFiles[i].getName() + "\\" + SiteLevel1FolderFiles[j].getName());
                        // To move file from one directory to another and if same name file exist in directory than it will replace.
                        Files.copy(original, destination, StandardCopyOption.REPLACE_EXISTING);
                    }
                    for (int k=0 ; k < SiteLevel2FolderFiles.length ; k++){
                        // To construct the path of file.
                        Path original = Paths.get(BasePath + Path + Sitelevel2 + "\\" + Sitelevel2FolderListOfFiles[i].getName() + "\\" + SiteLevel2FolderFiles[k].getName());
                        Path destination = Paths.get(BasePath + Path + Site + "_" + Sitelevel1 + "_" + Sitelevel2 + "\\" + Sitelevel1FolderListOfFiles[i].getName() + "\\" + SiteLevel2FolderFiles[k].getName());
                        // To move file from one directory to another and if same name file exist in directory than it will replace.
                        Files.copy(original, destination, StandardCopyOption.REPLACE_EXISTING);
                    }
                }
    ...
    public void WraithCommand() throws IOException {
        //Use the exec method of the Runtime class to run the command.
        //String newDir = "C:\\Users\\om\\Downloads\\Screenshots";
        //Runtime rt = Runtime.getRuntime();
        //rt.exec("cmd.exe /c start", null, new File(newDir));
        //rt.exec("wraith compare_images config_compare");
        //rt.exec("dir");
        System.out.println("rrrr");
        final String[] command = new String[]{"cmd", "/c",
                "\"C:\\Users\\om\\Downloads\\Screenshots\\echo hello\""};
